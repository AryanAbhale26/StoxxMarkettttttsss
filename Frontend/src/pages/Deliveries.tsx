...
const Deliveries = () => {
  ...
  const [deliveries, setDeliveries] = useState<StockMovement[]>([]);
  ...
  const loadDeliveries = async () => {
    try {
      const params: any = { movement_type: 'delivery' };
      if (filterStatus !== 'all') {
  ...
  const handleExecute = async (id: string) => {
    if (!confirm('Execute this delivery? Stock levels will be updated.')) {
      return;
    }
      ...
      <div className="p-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-1">Outgoing stock movements</p>
          </div>
          <button
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {deliveries.map((delivery) => (
                  <tr key={delivery.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {delivery.lines?.length || 0} item(s)
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      {delivery.status === 'ready' && (
                        <button
                          onClick={() => handleExecute(delivery.id)}
                      ...
                      <button
                        onClick={() => navigate(`/deliveries/view/${delivery.id}`)}
