...
import { TrendingDown, Plus, CheckCircle, Clock, XCircle } from 'lucide-react';
...
const Receipts = () => {
  ...
  const loadReceipts = async () => {
    try {
      const params: any = { movement_type: 'receipt' };
      if (filterStatus !== 'all') {
  ...
  const handleExecute = async (id: string) => {
    if (!confirm('Execute this receipt? Stock levels will be updated.')) {
      return;
    }
      ...
      <div className="p-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-1">Incoming stock movements</p>
          </div>
          <button
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {receipts.map((receipt) => (
                  <tr key={receipt.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {receipt.lines?.length || 0} item(s)
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      {receipt.status === 'ready' && (
                        <button
                          onClick={() => handleExecute(receipt.id)}
